...
class Production::InventoryReportItem < ActiveRecord::Base
  ...
  scope :by_on_hand_cost_desc, lambda { order('(inventory_report_items.cost * inventory_report_items.quantity_on_hand) desc') }
  scope :by_latest_activity, lambda { select("inventory_report_items.*,
