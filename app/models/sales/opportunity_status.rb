...
class Sales::OpportunityStatus < ActiveHash::Base
  self.data = [
    ...
    {:id => 6, :open => false, :name => 'Sales Rep', :cmethod => :sales_rep, :obsolete => true},
    {:id => 7, :open => false, :name => 'Closed'},
    {:id => 5, :open => false, :name => 'Deleted'}
  ]
  class << self
    def all_not_deleted
      @all_not_deleted ||= all.select { |s| !s.deleted? && !s.obsolete? }
    end
    def all_open
      @all_open ||= all.select(&:open?)
    end
    def all_closed
      @all_closed ||= all.select { |s| !s.open? && !s.obsolete? }
    end
  end
